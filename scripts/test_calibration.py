...
def record_calibration_points(world_frame_robot='/world',
...
def extract_transforms(flat_transforms):
    ...
    nb_transform = len(flat_transforms) 
    list_transforms = []
    for i in range(nb_transform):
        pose = []
        # extract the pose
        pose.append(flat_transforms[i:i + 3])
        pose.append(flat_transforms[i + 3:i + 7])
...
def evaluate_calibration(calibrations, coords_robot, coords_opt):
    ...
    for i in range(nb_points):
        ...
        X_trans_mat, X_rot_mat = get_pose_matrices(X[0], X[1])
        rob_trans_mat, rob_rot_mat = get_pose_matrices(robot[0], robot[1])
        opt_trans_mat, opt_rot_mat = get_pose_matrices(opt[0], opt[1])

        prod_trans_mat = np.dot(X_trans_mat, opt_trans_mat)
        prod_rot_mat = np.dot(X_rot_mat, opt_rot_mat)
        ...
        cost += distance_cost(robot, product)
    return cost
...
if __name__ == "__main__":
    ...
    calib_frame_robot = '/screen'  # The frame used for calibration in the robot's frame
    calib_frame_opt = '/optitrack_screen'  # The frame used for calibration in the optitrack's frame
    ...
    # print(multiply_A_B(calibration_matrix_a, calibration_matrix_b))
